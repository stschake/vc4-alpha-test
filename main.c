...
static int modeset_create_fb(int fd, struct modeset_dev *dev)
{
	...
	uint32_t bo_handles[4] = { dev->handle, 0, 0, 0 };
...
{
	unsigned int j, k, off, tux_off, tux_w, tux_h, tux_n;
	struct modeset_dev *iter;

	uint8_t* tux_data = stbi_load("tux.png", &tux_w, &tux_h, &tux_n, 4);
		...
		for (j = 0; j < tux_h; ++j) {
			for (k = 0; k < tux_w; ++k) {
				...
				*(uint32_t*)&iter->map[off] =
					(255 << 24) | // A
					(tux_data[tux_off+0] << 16) | // R
					(tux_data[tux_off+1] << 8) |  // G
					(tux_data[tux_off+2]);        // B 
			}
		}
	}
